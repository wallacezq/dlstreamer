...
static gboolean gst_analytics_keypoint_mtd_meta_transform(GstBuffer *transbuf, GstAnalyticsMtd *transmtd,
    ...
    if (GST_VIDEO_META_TRANSFORM_IS_MATRIX(type)) {
        ...
        kpdata = gst_analytics_relation_meta_get_mtd_data(transmtd->meta, transmtd->id);

        GST_DEBUG("Keypoint scale transform: in=(%ux%u) out=(%ux%u)", ow, oh, nw, nh);
        ...
        GST_DEBUG("Keypoint scaled: (%d,%d) in frame (%u,%u) -> (%u,%u)", kpdata->x, kpdata->y, ow, oh, nw, nh);
    }
...
gboolean gst_analytics_relation_meta_add_keypoint_mtd(GstAnalyticsRelationMeta *instance,
    ...
    GstAnalyticsKeypointMtdData *keypoint_data = (GstAnalyticsKeypointMtdData *)gst_analytics_relation_meta_add_mtd(
        instance, &keypoint_impl, sizeof(GstAnalyticsKeypointMtdData), keypoint_mtd);
...
gboolean gst_analytics_keypoint_mtd_get_position(const GstAnalyticsKeypointMtd *handle, gint *x, gint *y, gint *z,
    ...
    keypoint_data = gst_analytics_relation_meta_get_mtd_data(handle->meta, handle->id);
 ...
 * @visibilities: (array length=keypoint_count) (nullable): Array of #GstAnalyticsKeypointVisibility flags [v0, v1, ...]
 * or NULL
 * @skeleton_pairs_len: Length of @skeleton_pairs array (must be even, representing pairs of keypoint indices)
 * @skeleton_pairs: (array length=skeleton_pairs_len) (nullable): Array of keypoint index pairs [kp1_idx, kp2_idx,
 * kp1_idx, kp2_idx, ...] or NULL
...
gboolean gst_analytics_relation_meta_add_keypoints_group(GstAnalyticsRelationMeta *instance, const gchar *semantic_tag,
    ...
    g_return_val_if_fail((positions_len % _GET_KEYPOINT_STRIDE(dimension)) == 0, FALSE);
    ...
    ret = gst_analytics_relation_meta_add_group_mtd_with_size(instance, keypoint_count, group_mtd);
    if (!ret) {
        return FALSE;
    }
    ...
    for (i = 0; i < keypoint_count; i++) {
        ...
        guint8 visibility = visibilities ? visibilities[i] : GST_ANALYTICS_KEYPOINT_VISIBILITY_UNKNOWN;

        /* Add keypoint metadata which may trigger buffer reallocation */
        ret =
            gst_analytics_relation_meta_add_keypoint_mtd(instance, dimension, x, y, z, visibility, confidence, &kp_mtd);
        if (!ret) {
    ...
    if (skeleton_pairs && skeleton_pairs_len > 0) {
        ...
        if (skeleton_pairs_len % 2 != 0) {
            GST_WARNING("skeleton_pairs_len must be even (got %zu)", skeleton_pairs_len);
            g_free(keypoint_ids);
            return FALSE;
        }
        ...
        for (i = 0; i < skeleton_count; i++) {
            ...
            if (kp1_idx >= keypoint_count || kp2_idx >= keypoint_count) {
                GST_WARNING("Invalid skeleton link indices: %u, %u (keypoint_count: %zu)", kp1_idx, kp2_idx,
                            keypoint_count);
                g_free(keypoint_ids);
                return FALSE;
            }
            ...
            ret = gst_analytics_relation_meta_set_relation(instance, GST_ANALYTICS_REL_TYPE_RELATE_TO,
                                                           keypoint_ids[kp1_idx], keypoint_ids[kp2_idx]);
            if (!ret) {
                GST_WARNING("Failed to create skeleton link between %u and %u", kp1_idx, kp2_idx);
                g_free(keypoint_ids);
                return FALSE;
            }
        }
